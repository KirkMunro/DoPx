...
# version of DoPx from the GitHub page where it is hosted.
...
try {
    ...
    Write-Progress -Activity 'Installing DoPx' -Status 'Defining common Windows PowerShell modules folder paths.'
    $modulesFolders = @{
    ...
    Write-Progress -Activity 'Installing DoPx' -Status 'Identifying the target modules folder.'
    ...
    if (-not (Test-Path -LiteralPath $modulesFolder)) {
        Write-Progress -Activity 'Installing DoPx' -Status 'Creating modules folder.'
        New-Item -Path $modulesFolder -ItemType Directory -ErrorAction Stop > $null
    }
    if (@($env:PSModulePath -split ';') -notcontains $modulesFolder) {
        Write-Progress -Activity 'Installing DoPx' -Status 'Updating the PSModulePath environment variable.'
    ...
    Write-Progress -Activity 'Installing DoPx' -Status 'Downloading the latest version of DoPx.'
    $zipFilePath = Join-Path -Path $modulesFolder -ChildPath DoPx.zip
    $response = Invoke-WebRequest -Uri https://github.com/KirkMunro/DoPx/zipball/release -ErrorAction Stop
    ...
    Write-Progress -Activity 'Installing DoPx' -Status 'Extracting the DoPx zip file contents.'
    ...
    Write-Progress -Activity 'Installing DoPx' -Status 'Removing the DoPx zip file.'
    ...
    if ($module) {
        Write-Progress -Activity 'Installing DoPx' -Status 'Unloading and removing the installed DoPx module.'
    ...
    Write-Progress -Activity 'Installing DoPx' -Status 'Installing the new DoPx module.'
    Join-Path -Path $modulesFolder -ChildPath KirkMunro-DoPx-* `
        | Get-Item `
        | Sort-Object -Property LastWriteTime -Descending `
        | Select-Object -First 1 `
        | Rename-Item -NewName DoPx
    ...
    if ($PSCmdlet.MyInvocation.BoundParameters.ContainsKey('PassThru') -and $PassThru) {
        Get-Module -ListAvailable -Name DoPx
    }
...
# BgkqhkiG9w0BCQUxDxcNMTQxMTA4MjMzMzE5WjAjBgkqhkiG9w0BCQQxFgQUs37x
# Lht/I1hrSDA5AzRJN/jy6OgwDQYJKoZIhvcNAQEBBQAEggEAEzIG0ewItpfVouiK
# XRHZszkcyiMviBH667chzrOsG6BDprFWOoslc4fJK2Q2ASF5nNgiVSPIj7N+3L6v
# wWv+aqgJJUj1i/JmZMUTX6l6HRnjcSiISYZGy6S1sbhER/YrLcjtjSBiPAZUPtzY
# 39tHS6u3n9+PBVQk6TVzvv2raQhnMjPAjsIemtBsL4RMt5pjkpjofoUbbvqV5Gsh
# zIDZeEpfBxb3I9RnOOdvxd6BhirwHL5dydjkLhnoKdPUQW5txrOViRbwTBexG5ES
# 2E82bbg+60p3v1bdlI+bJqoIMMHmNDFbgX0atG5jJZTW/7aNbWAuy8l0T+GMLkqN
# D7/6Rw==
